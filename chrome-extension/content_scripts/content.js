(function() {
    ...
    function handleDialog(ev) {
        if (ev.target.classList.contains('better-image-description-button')) {
            ...
            window.a11yDialog.on('show', function() {
    ...
    function removeChecks(hideDialog = true) {
    ...
    function onScroll() {
        removeChecks(hideDialog = false);
        checkAllImages();
    }

    function insertCSS() {
        if (!document.querySelector('#bdi-css-link')) {
            const link = document.createElement('link');
            link.rel = 'stylesheet';
            link.type = 'text/css';
            link.id = 'bdi-css-link';
            link.href = chrome.extension.getURL('/content_scripts/content.css');
            document.body.appendChild(link);
        }
    }

    function removeCSS() {
        const stylesheet = document.querySelector('#bdi-css-link');
        if (stylesheet) {
